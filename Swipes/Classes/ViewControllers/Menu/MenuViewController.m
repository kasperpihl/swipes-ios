...
-(void)pushViewController:(UIViewController*)viewController animated:(BOOL)animated{
    ...
    [UIView animateWithDuration:0.1 animations:^{
        hidingView.alpha = 0;
        if (level == 0)
            [self.backButton setImage:[UIImage imageNamed:timageStringBW(@"round_cross")] forState:UIControlStateNormal];
    } completion:^(BOOL finished) {
...
-(UIImage *)imageForMenuButton:(KPMenuButtons)button highlighted:(BOOL)highlighted{
    ...
    if (highlighted)
        imageString = [imageString stringByAppendingString:@"-high"];
